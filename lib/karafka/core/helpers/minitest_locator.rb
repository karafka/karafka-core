...
module Karafka
  module Core
    module Helpers
      # Minitest::Spec extension for the `describe` subject class auto-discovery.
      ...
      #   instead of `describe`
      #   Kernel.extend Karafka::Core::Helpers::MinitestLocator.new(__FILE__)
      class MinitestLocator < Module
        ...
        # Builds needed API
        # @param base [Module] module being extended (typically Kernel)
        def extended(base)
          super
          this = self
          ...
          base.define_method :describe_current do |&block|
            describe(this.inherited, &block)
          end
          base.send(:private, :describe_current)
        end
