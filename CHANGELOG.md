...
## 2.6.2 (Unreleased)
- [Fix] `Contract#call` no longer mutates the array a virtual rule returns when applying the scope prefix. A virtual rule that returns a memoized/reused array previously had its entries rewritten in place, accumulating the scope prefix on every validation (and corrupting the caller's array); the scoped error entries are now built fresh.
