...
## 2.6.2 (Unreleased)
- [Fix] Iterate over a snapshot of the listeners in `Notifications#instrument` so a listener that unsubscribes itself (or an earlier listener) from within its own handler no longer causes the listener following it to be silently skipped. Previously the live listeners array was mutated mid-iteration, which also made concurrent subscribe/unsubscribe during dispatch unsafe.
