...
- [Fix] `StatisticsDecorator` no longer raises `TypeError` when a key's value changes from a `Numeric` to a `Hash` between emissions. The recursion treated the previous (non-Hash) value as a hash and indexed it with a key (`5["y"]`); it now treats a non-Hash previous as "no previous", mirroring the existing scalar type-change defense, in both the default and `only_keys` paths.
