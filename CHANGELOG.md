...
## 2.6.1 (2026-06-11)
- [Change] Reject reserved setting names (`node_name`, `children`, `nestings`, `compiled`, `configs_refs`, `local_defs`) with an `ArgumentError` in `Configurable::Node#setting` and `#register`. Previously such names silently shadowed the node own accessors, breaking `deep_dup`, and assignments like `config.children = value` corrupted the node internal state.
